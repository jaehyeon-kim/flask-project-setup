...
def docker_compose_file(config):
    return os.path.join(DOCKER_PATH, f"{config}.yaml")
...
def configure_app(config):
    ...
    config_data = {c["name"]: c["value"] for c in config_data}
...
def docker_compose_cmdline(commands_string=None):
    ...
    if not os.path.isfile(compose_file):
        raise ValueError(f"The file {docker_compose_file} doest not exist")
...
def compose(subcommand):
    ...
    print("The following docker-compose command is executed")
    print(" ".join(cmdline))
